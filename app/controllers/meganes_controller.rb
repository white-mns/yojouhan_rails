class MeganesController < ApplicationController
  ...
  def index
    param_set
    @count	= Megane.includes([:p_name, :megane_type_name]).search(params[:q]).result.count()
    @search	= Megane.includes([:p_name, :megane_type_name]).page(params[:page]).search(params[:q])
